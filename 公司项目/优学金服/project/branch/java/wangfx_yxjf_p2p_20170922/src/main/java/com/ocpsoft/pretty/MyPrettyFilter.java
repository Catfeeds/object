...
public class MyPrettyFilter implements Filter {
	...
	public static final String CONFIG_KEY = "com.zw.urlMapping.CONFIG_FILES";
	...
	public void doFilter(final ServletRequest req, final ServletResponse resp,
		...
		if (resp.isCommitted()) {
			log.info(">>Rewrite occurred, reponse is committed - ending request.");
		} else {
			...
			if (mapping != null) {
				...
				if (!response.isCommitted()) {
					if (context.shouldProcessDynaview()) {
						log.info(">>Forwarding mapped request [" + url.toURL()
						...
						log.info(">>Sending mapped request \n\t[" + url.toURL()
								+ "] \n\tto resource \n\t[" + viewId+ query + "]");
...
//			hreq.getSession(true).setAttribute(PrettyContext.CONFIG_KEY, pc);
	...
	private PrettyConfig handleUserConfig(PrettyConfig pc,
		...
		List<com.zw.archer.urlmapping.model.UrlMapping> dbMappings = ht
				.loadAll(com.zw.archer.urlmapping.model.UrlMapping.class);
		for (com.zw.archer.urlmapping.model.UrlMapping urlMapping : dbMappings) {
