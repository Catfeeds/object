...
import com.zw.archer.lucene.LuceneConstants;
import com.zw.archer.lucene.model.Indexing;
import com.zw.archer.lucene.service.LuceneService;
import com.zw.archer.node.NodeConstants;
import com.zw.archer.node.model.Node;
import com.zw.archer.node.model.NodeBody;
import com.zw.archer.node.model.NodeType;
import com.zw.archer.node.service.NodeService;
...
public class NodeHome extends EntityHome<Node> implements Serializable {
	...
	public String save() {
		...
		if (node.getId()==null)
			node.setId(java.util.UUID.randomUUID().toString().replaceAll("-","").substring(0,32));
...
//		return null;
	 ...
	 * @param event
	 */
	public void handleNodePicturesUpload(FileUploadEvent event) {
		UploadedFile uploadFile = event.getFile();
		InputStream is = null;
		try {
			is = uploadFile.getInputstream();
			this.getInstance().setNodepic(ImageUploadUtil.upload(is, uploadFile.getFileName()));
		} catch (IOException e) {
			e.printStackTrace();
			return;
		}
