...
public class DictUtil implements java.io.Serializable{
     ...
     * @return
     */
    public static Integer getMaxDeadLineValue(String parentKey ,String key){
        String limitType =getValue(parentKey, key);
        if(StringUtils.isNotEmpty(limitType)){
            if("9".equals(limitType)){
                return 366*3;
            }
            if("月".equals(limitType)){
                return 126*3;
            }
        }else {
            return  0;
