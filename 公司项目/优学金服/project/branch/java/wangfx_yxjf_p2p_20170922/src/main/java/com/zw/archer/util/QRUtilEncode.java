...
public class QRUtilEncode {
	...
	public static boolean encode_84(String srcValue, String qrcodePicfilePath) {
		...
		if (dataLength > 0 && dataLength <= MAX_DATA_LENGTH) {
			...
			qrcode.setQrcodeErrorCorrect('H'); // L, Q, H, 默认
			qrcode.setQrcodeEncodeMode('B'); // A, N, 默认
			//qrcode.setQrcodeVersion(4); // 37字节, (37-1)*3+2+3-1+1 = 113
			boolean[][] b = qrcode.calQrcode(d);
			int qrcodeDataLen = b.length;
			int imageWidth=(qrcodeDataLen-1)*3+2+3;
	...
	public static boolean encode_500(String srcValue, String qrcodePicfilePath) {
		...
		if (dataLength > 0 && dataLength <= MAX_DATA_LENGTH) {
			/* 生成二维码 */
			Qrcode qrcode = new Qrcode();
			qrcode.setQrcodeErrorCorrect('M'); // L, Q, H, 默认M
			qrcode.setQrcodeEncodeMode('B'); // A, N, 默认B
	...
	QRUtilEncode.encode_500(data, "d://test500111.JPG");
	QRUtilEncode.encode_84(data, "d://test84111.JPG");
	/*QRUtil2.encode_500(data, "d://B.JPG");*/
		//QRUtil2.encode_841(data, "d://test841.JPG");
	}
}