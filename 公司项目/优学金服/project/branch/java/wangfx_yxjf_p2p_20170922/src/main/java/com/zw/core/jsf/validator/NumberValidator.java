...
//@Component
@FacesValidator(value = "com.zw.core.validator.NumberValidator")
public class NumberValidator extends ValueChangeValidator implements
		PartialStateHolder {

	public static final String VALIDATOR_ID = "com.zw.core.validator.NumberValidator";
	...
	public void validateChangedObject(FacesContext context,
		...
		if (number != null) {
			checkMaxNumber(context, component, number);
			checkMinNumber(context, component, number);
			checkPrecision(context, component, number);
			checkCardinalNumber(context, component, number);
		}

	}
	private void checkMinNumber(FacesContext context,
		...
		if (minimum != null) {
			...
			if (number <minimum ) {
				// 验证未通过
				// TODO:国际化
				throw new ValidatorException(Messages.createError("{0}不可小于最小可投金额",
						Components.getLabel(component)));
			}
		}
	}

	private void checkMaxNumber(FacesContext context,
			UIComponent component, Double number) {
		if (maximum != null) {
			if (maximum == 0) {
				// 除数为0，抛异常
			...
			if (number >maximum ) {
				// 验证未通过
				// TODO:国际化
				throw new ValidatorException(Messages.createError("{0}不可大于剩余可投金额",
						Components.getLabel(component)));
			}
		}
	}
			...
			if (precision == 0) {
				if (ns.length != 1) {
					throw new ValidatorException(
							MessageFactory
									.getMessage(
											context,
											"com.zw.core.validator.NumberValidator.NOT_A_INTEGER",
				...
				throw new ValidatorException(
						MessageFactory
								.getMessage(
										context,
										"com.zw.core.validator.NumberValidator.ERROR_PRECISION",
										MessageFactory.getLabel(context,
												component), precision));
			}
		}
