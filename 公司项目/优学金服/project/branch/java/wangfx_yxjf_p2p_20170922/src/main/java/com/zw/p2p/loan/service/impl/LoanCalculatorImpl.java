...
import com.zw.archer.config.controller.ConfigHome;
...
import com.zw.p2p.repay.service.impl.NormalRepayCPMCalculator;
import com.zw.p2p.repay.service.impl.NormalRepayRFCLCalculator;
import com.zw.p2p.repay.service.impl.NormalRepayRLIOCalculator;
...
public class LoanCalculatorImpl implements LoanCalculator {
	...
	public String calculateRemainTime(String loanId)
		...
		if (time < 0 ) {
			return "已到期";
		}else if (!loan.getStatus().equals(LoanStatus.RAISING)){
			return "0天0小时0分";
		}
...
//		remain = remain < loan.getMaxInvestMoney() ? remain :loan.getMaxInvestMoney();
	...
	public double calculateRepayMoney(Loan loan){
		double money = 0D;
		String hql = "from LoanRepay lr where lr.loan.id = ? and lr.status = 'complete'";
		List<LoanRepay> lrs = ht.find(hql,loan.getId());
		for (LoanRepay lr : lrs){
			money += lr.getCorpus() + lr.getInterest();
		}
		return money;
	}
	 ...
	 * @return
	 */
	public double getTotalRepayMoney(Loan loan){
		double totalMoney = 0D;
		String hql = "from LoanRepay lr where lr.loan.id = ?";
		List<LoanRepay> lrs = ht.find(hql,loan.getId());
		for (LoanRepay lr :lrs){
			totalMoney += lr.getCorpus() + lr.getInterest();
		}
		return  totalMoney;
	}
