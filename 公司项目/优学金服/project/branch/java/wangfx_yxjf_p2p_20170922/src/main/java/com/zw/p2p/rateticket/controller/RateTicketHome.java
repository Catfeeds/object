...
public class RateTicketHome extends EntityHome<RateTicket> implements Serializable {
	...
	public RateTicketHome() {
		setUpdateView(FacesUtil.redirect("/admin/rateTicket/rateTicketList"));
	}
	
	@Override
	@Transactional(readOnly = false)
	public String save() {
		String rateticketAdminCreateTime = configService.getConfigValue("rateticketAdminCreateTime");//每种加息券的发放开始日期和结束日期可单独配置
		String rateticketAdminInvestEndTime = configService.getConfigValue("rateticketAdminInvestEndTime");
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		Date now = new Date();
		try {
			if(now.compareTo(sdf.parse(rateticketAdminCreateTime))>0 && sdf.parse(rateticketAdminInvestEndTime).compareTo(now)>0){
				getInstance().setId( UUID.randomUUID().toString().replaceAll("-", ""));
				getInstance().setCreateTime(new Date());
				getInstance().setResource(RateTicketConstants.RESOURCETYPE.admindo.getName());
				getInstance().setStatus(RateTicketConstants.RateTicketStatus.ENABLE);
			...
			while(row != null){
				...
				if(null != row){
					...
					try {
						...
						getInstance().setRate(row.getCell(1).getNumericCellValue());
...
//            }
		...
		//FIXME: 删除源文件
		return FacesUtil.redirect("/admin/rateTicket/rateTicketList") ;
	}
