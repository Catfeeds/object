...
public class NormalRepayRFCLCalculator implements NormalRepayCalculator {
	...
	public List<Repay> generateRepays(double investMoney, Date investTime,
		...
		if (interestType.equals(InterestType.DAY)) {
			...
			if (repayTimeUnit.equals(RepayUnit.MONTH)) {
				// 按月s还款
				return generateMonthMonthRepays(investMoney, investTime, rate,
						deadline, repayTimePeriod, interestBeginTime,
						interestBeginPoint);
				/*throw new IllegalLoanTypeException("interestType: "
		...
		for (int i = 1; i <= deadline; i++) {
			...
			int interestDays = DateUtil.getIntervalDays(
					DateUtil.addMonth(interestBeginTime, (i - 1)
							* repayTimePeriod),
					DateUtil.addMonth(interestBeginTime, i * repayTimePeriod));
			if (i == 1
					&& interestBeginPoint
							.equals(InterestPoint.INTEREST_BEGIN_ON_INVEST)) {
			...
			ir.setInterest(ArithUtil.round(rate / 365 * interestDays
					* investMoney, 2));
	 ...
	 *            计息节点（即投即生息，放款后生息 之类）
	 */
	private List<Repay> generateMonthMonthRepays(double investMoney,
			Date investTime, double rate, Integer deadline,
			Integer repayTimePeriod, Date interestBeginTime,
			String interestBeginPoint) {
		List<Repay> repays = new ArrayList<Repay>();
		for (int i = 1; i <= deadline; i++) {
			...
			// 金额*还款周期*还款周期单位*还款周期单位利率 = 利息
			ir.setInterest(ArithUtil.round(rate / 12 * repayTimePeriod
					* investMoney, 2));
			ir.setLength(repayTimePeriod);
			ir.setPeriod(i);
			// 计息日+第几期*还款周期=还款日
	...
	private List<Repay> generateDayDayRepays(double investMoney,
		...
		for (int i = 1; i <= deadline; i++) {
			...
			ir.setPeriod(i);
			// 计息日+第几期*还款周期单位=还款日
			ir.setRepayDay(DateUtil.addDay(interestBeginTime, i
					* repayTimePeriod));
			repays.add(ir);
		}
		return repays;
