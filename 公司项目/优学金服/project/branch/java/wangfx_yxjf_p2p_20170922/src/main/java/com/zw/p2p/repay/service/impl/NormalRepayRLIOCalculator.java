...
public class NormalRepayRLIOCalculator implements NormalRepayCalculator {
	...
	public List<Repay> generateRepays(double investMoney, Date investTime,
		...
		if (interestType.equals(InterestType.DAY)) {
			...
			if (repayTimeUnit.equals(RepayUnit.MONTH)) {
				// 按月s还款
				return generateMonthMonthRepays(investMoney, investTime, rate, deadline, repayTimePeriod, interestBeginTime, interestBeginPoint);
	...
	private List<Repay> generateMonthMonthRepays(double investMoney,
		...
		// 金额*还款周期*还款周期单位*还款周期单位利率 = 利息
		ir.setInterest(ArithUtil.round(rate / 12 * deadline * investMoney,
				2));
		ir.setLength(deadline);
		ir.setPeriod(1);
		// 计息日+第几期*还款周期=还款日
